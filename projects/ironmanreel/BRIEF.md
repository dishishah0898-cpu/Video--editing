...
- One line of copy only — "How's your Ironman training?" over the squat shot
  (1.8-5.8s), then her footage runs uninterrupted. The per-discipline chapter
  marks and the mid statement are `data-hidden`, not deleted.
...
- Graphics dialled back on request: the viewfinder brackets and the status chip are
  ...
  — corner mark 96 to 66px, rails 5 to 3px. What is left on screen is the one
  copy line, the corner mark, the progress rail, and the swim plate's hairlines.
